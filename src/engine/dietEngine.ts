...
export function calculateDietMetrics(profile:UserProfile,goal:DietGoal):CalculatedDietMetrics{
  ...
  const targetCalories=Math.max(1200,goal==='hypertrophy'?tdee+250:goal==='cutting'?tdee-400:tdee);
  ...
  let carbGrams=Math.max(0,Math.round((targetCalories-proteinCalories-fatGrams*9)/4));
  if(carbGrams<80&&targetCalories-proteinCalories-fatGrams*9>=0)carbGrams=80;
  let fatCalories=fatGrams*9;
  let carbCalories=carbGrams*4;
  let total=proteinCalories+fatCalories+carbCalories;
  if(total>targetCalories){
    carbGrams=Math.max(0,Math.floor((targetCalories-proteinCalories-fatCalories)/4));
    carbCalories=carbGrams*4;
    total=proteinCalories+fatCalories+carbCalories;
    if(total>targetCalories){ fatGrams=Math.max(0,Math.floor((targetCalories-proteinCalories-carbCalories)/9)); fatCalories=fatGrams*9; }
  } else if(total<targetCalories){
    const extra=Math.round((targetCalories-total)/9);
    fatGrams+=Math.max(0,extra); fatCalories=fatGrams*9;
  }
  carbCalories=Math.max(0,targetCalories-proteinCalories-fatCalories);
  carbGrams=Math.floor(carbCalories/4);
...
function scaleFood(food:BaseFood,factor:number):FoodItem{const amount=food.amount.replace(/^([0-9]+(?:\.[0-9]+)?)/,(m)=>round1(Number(m)*factor).toString());return {...food,amount,calories:Math.round(food.calories*factor),protein:round1(food.protein*factor),carbs:round1(food.carbs*factor),fat:round1(food.fat*factor)};}
...
function buildMeal(id:string,name:string,time:string,type:Meal['type'],targetCalories:number,baseFoods:BaseFood[],tips:string):Meal{const raw=totals(baseFoods);const factor=raw.calories>0?targetCalories/raw.calories:1;const foods=baseFoods.map(f=>scaleFood(f,factor));const t=totals(foods);return{id,name,time,type,calories:t.calories,protein:round1(t.protein),carbs:round1(t.carbs),fat:round1(t.fat),foods,tips};}
...
function rebalanceMealCalories(meal:Meal,targetCalories:number,carbScale:number):Meal{
  const carbFoods=meal.foods.filter(f=>f.category==='carb').map(f=>scaleFood(f,carbScale));
  const otherFoods=meal.foods.filter(f=>f.category!=='carb');
  const carbTotal=totals(carbFoods).calories;
  const otherTotal=totals(otherFoods).calories;
  const desiredOther=Math.max(0,targetCalories-carbTotal);
  const otherScale=otherTotal>0?desiredOther/otherTotal:1;
  const foods=[...carbFoods,...otherFoods.map(f=>scaleFood(f,otherScale))];
  const t=totals(foods);
  return {...meal,foods,calories:t.calories,protein:round1(t.protein),carbs:round1(t.carbs),fat:round1(t.fat)};
}

export function generateMealPlan(profile:UserProfile,goal:DietGoal,preference:DietPreference):StructuredDietPlan{
  const metrics=calculateDietMetrics(profile,goal);const ratios=[0.22,0.30,0.20,0.22,0.06];const names=['Refeição 1 — Café da Manhã','Refeição 2 — Almoço','Refeição 3 — Pré-Treino','Refeição 4 — Jantar','Refeição 5 — Ceia'];const times=['07:30','12:30','16:00','19:30','22:00'];const types:Meal['type'][]=['breakfast','lunch','pre_workout','dinner','supper'];const tips=preference==='low_carb'?'Priorize vegetais, proteína e gorduras adicionadas com porções compatíveis com sua meta diária.':preference==='low_cost'?'Priorize alimentos acessíveis e varie as fontes conforme preço e disponibilidade.':preference==='practical'?'Prefira opções simples de preparar e transportar, mantendo as porções registradas.':preference==='vegetarian'?'Combine fontes vegetais e, quando aplicável, ovos e laticínios para atingir a meta proteica.':'Use porções compatíveis com a meta diária e ajuste conforme sua evolução e preferência.';let meals=ratios.map((ratio,i)=>buildMeal(`meal-${i+1}`,names[i],times[i],types[i],Math.round(metrics.targetCalories*ratio),buildBaseMealFoods(preference,i),tips));
  if(preference==='low_carb'){const carbCap=Math.min(metrics.carbGrams,Math.floor(metrics.targetCalories*0.20/4));const carbScale=metrics.carbGrams>0?carbCap/metrics.carbGrams:0;meals=meals.map((meal,i)=>rebalanceMealCalories(meal,Math.round(metrics.targetCalories*ratios[i]),carbScale));}
...
export const FOOD_SUBSTITUTIONS_DATABASE:SubstitutionGroup[]=[
 {category:'carb',title:'Fontes de carboidratos',baseFood:'100g de arroz cozido',equivalents:[{foodName:'Arroz branco/integral cozido',portion:'100g'},{foodName:'Batata cozida/assada',portion:'150g'},{foodName:'Mandioca cozida',portion:'90g'},{foodName:'Aveia',portion:'40g'},{foodName:'Banana',portion:'1 unidade média'},{foodName:'Pão integral',portion:'2 fatias'}]},
 {category:'protein',title:'Fontes de proteína',baseFood:'100g de frango grelhado',equivalents:[{foodName:'Frango grelhado',portion:'100g'},{foodName:'Peixe magro',portion:'110g'},{foodName:'Ovos',portion:'3 unidades'},{foodName:'Tofu firme',portion:'150g'},{foodName:'Whey protein',portion:'30g'},{foodName:'Iogurte proteico',portion:'160g'}]},
 {category:'fat',title:'Fontes de gordura',baseFood:'10ml de azeite',equivalents:[{foodName:'Azeite de oliva',portion:'10ml'},{foodName:'Pasta de amendoim',portion:'15g'},{foodName:'Castanhas',portion:'15g'},{foodName:'Abacate',portion:'70g'}]},
 {category:'fiber',title:'Vegetais e fibras',baseFood:'Porção de legumes e folhas',equivalents:[{foodName:'Folhas variadas',portion:'à vontade'},{foodName:'Brócolis',portion:'150g'},{foodName:'Cenoura',portion:'100g'},{foodName:'Repolho',portion:'150g'},{foodName:'Abóbora',portion:'150g'}]},
];
...
export function nutritionPlanIsInternallyConsistent(plan:StructuredDietPlan,tolerance=2){const t=calculatePlanTotals(plan);const declared=plan.planTotals||t;return Math.abs(t.calories-declared.calories)<=tolerance&&plan.meals.every(meal=>{const x=totals(meal.foods);return Math.abs(x.calories-meal.calories)<=1&&Math.abs(x.protein-meal.protein)<=1&&Math.abs(x.carbs-meal.carbs)<=1&&Math.abs(x.fat-meal.fat)<=1;});}
