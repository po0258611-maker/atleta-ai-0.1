...
function recomputeProgramMetrics(splitDays: FullBodyProgram['splitDays']) {
  ...
  splitDays.forEach((day) => {
    ...
    day.systemicFatigueScore = maxFatigue > 0 ? Math.round((weightedFatigue / maxFatigue) * 100) : 0;
  });

  return { weeklyVolumeMap, frequencyMap };
}
...
export function generateFullBodyWorkout(rawProfile: UserProfile): FullBodyProgram {
  ...
  const splitDays = base.splitDays.map((day) => ({
    ...day,
    items: rebalanceDay(day.items, target, priorities, budget),
  }));
  ...
  const { weeklyVolumeMap, frequencyMap } = recomputeProgramMetrics(splitDays);
